...
# ---------------- CONFIG ----------------
CSV_PATH = "/content/AdharaAI/adharaai/Data/adharaai_deployment_ready_v3.csv"
...
df["is_synth"] = df["source_file"].astype(str).str.contains("synthetic", case=False)
...
print("Baselines: v2 script  risk 0.385 / type 0.682 | v3 script on 154 rows  risk 0.521 / type 0.808")
...
real = sum([r["real"] for r in all_results], [])
tt_r = [x for x, m in zip(tt, real) if m]; tp_r = [x for x, m in zip(tp, real) if m]
rt_r = [x for x, m in zip(rt, real) if m]; rp_r = [x for x, m in zip(rp, real) if m]
print(f"\nREAL-ROWS-ONLY pooled ({len(tt_r)} of {len(tt)} predictions): "
      f"type_f1={f1_score(tt_r, tp_r, average='macro', zero_division=0):.3f}  "
      f"risk_f1={f1_score(rt_r, rp_r, average='macro', zero_division=0):.3f}")
